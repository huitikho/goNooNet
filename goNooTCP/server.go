...
import (
	"encoding/hex"
	"encoding/json"
	"goNooNet/goNooTCP/client"
	"goNooNet/goNooTCP/tools"
	"goNooNet/helpers"
	"log"
	"net"
	"runtime"
...
func getActiveNodes() (nodes []tools.Node, err error) {
	...
	if helpers.IsRedisError(answer) {
		//
		...
		case data := <-readChannel:
			...
			if tranType == "HL" {
				...
				if simpleTransaction, err := helpers.ParseSimpleTransaction(data); err == nil {
					...
					if transactionTime, ok := helpers.VerifySimpleTransaction(simpleTransaction); ok {
						//
